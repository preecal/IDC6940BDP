...
## Impact of Age and MR Delay on Brain Volume (nWBV)
...
-   it determines the parameters under which the observed data is most
    probable.

-   estimates both the fixed effects (population-level parameters) and
    variance components (random effects and residual variances) by
...
-   maximizes the likelihood of the data after adjusting for the fixed
    effects, focusing on variance components estimation.

-   less biased because it adjusts for the loss of degrees of freedom
    caused by estimating fixed effects.
